    ...
    > AdvStats<M, D>
{
    pub(crate) fn get_filter_row_cnt(
        ...
        let selectivity = {
            let column_refs = output_column_refs.base_table_column_refs();
            self.get_filter_selectivity(cond, &output_schema, column_refs)
        };
    ...
    pub(super) fn get_filter_selectivity(
        ...
        match &expr_tree.typ {
            ...
            DfPredType::Between => UNIMPLEMENTED_SEL,
            DfPredType::Cast => unimplemented!("check bool type or else panic"),
            DfPredType::Like => {
    ...
    fn get_constant_selectivity(const_node: ArcDfPredNode) -> f64 {
        if let DfPredType::Constant(const_typ) = const_node.typ {
    ...
    fn get_semantic_nodes(
        ...
        right: ArcDfPredNode,
        schema: &Schema,
    ) -> (Vec<ColumnRefPred>, Vec<Value>, Vec<ArcDfPredNode>, bool) {
        ...
        loop {
            ...
            } else if uncasted_left.as_ref().typ == DfPredType::Cast
                || uncasted_right.as_ref().typ == DfPredType::Cast
            {
                let is_left_cast = uncasted_left.as_ref().typ == DfPredType::Cast;
                ...
                let should_break = match cast_expr_child.typ {
                    ...
                    DfPredType::ColumnRef => {
                        let col_ref_expr = ColumnRefPred::from_pred_node(cast_expr_child)
        ...
        match uncasted_left.as_ref().typ {
            DfPredType::ColumnRef => {
                ...
                col_ref_exprs.push(
                    ColumnRefPred::from_pred_node(uncasted_left)
                        .expect("we already checked that the type is ColumnRef"),
                );
            }
            DfPredType::Constant(_) => {
        ...
        match uncasted_right.as_ref().typ {
            DfPredType::ColumnRef => {
                col_ref_exprs.push(
                    ColumnRefPred::from_pred_node(uncasted_right)
                        .expect("we already checked that the type is ColumnRef"),
                );
            }
            DfPredType::Constant(_) => values.push(
                ...
                &column_refs[col_ref_idx]
            {
                if values.len() == 1 {
                    ...
                    match non_col_ref_expr.as_ref().typ {
                        ...
                        DfPredType::Cast => UNIMPLEMENTED_SEL,
                        DfPredType::Constant(_) => unreachable!(
